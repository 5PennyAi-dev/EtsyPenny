...
import { Star, Search, Gem, Sparkles, X, Loader2, Check, Folder, Hash, BarChart3 } from 'lucide-react';
...
const FavoritesPickerModal = ({ isOpen, onClose, onAddBatchKeywords, isAddingBatch, user, currentListing, existingKeywords = new Set() }) => {
  ...
  // Fetch bank + gem settings + presets when modal opens
  useEffect(() => {
    ...
    const fetchData = async () => {
      setIsLoading(true);
      const [bankRes, settingsRes, presetsRes] = await Promise.all([
  ...
  // Resolve presets: compute tag count and total volume from bank keyword IDs
  const resolvedPresets = useMemo(() => {
    if (!presets.length || !bankKeywords.length) return presets.map(p => ({ ...p, tagCount: (p.keyword_ids || []).length, totalVolume: 0, resolvedKeywords: [] }));
    const bankById = {};
    bankKeywords.forEach(kw => { bankById[kw.id] = kw; });

    return presets.map(preset => {
      const ids = preset.keyword_ids || [];
      const resolved = ids.map(id => bankById[id]).filter(Boolean);
      const totalVolume = resolved.reduce((sum, kw) => sum + (kw.last_volume || 0), 0);
      return {
        ...preset,
  ...
  }, [presets, bankKeywords]);

  if (!isOpen) return null;

  // Filter logic (favorites tab)
  const isGem = (kw) => {
  ...
  const handleSubmit = async () => {
    if (!onAddBatchKeywords) return;

    if (activeTab === 'presets') {
      // Preset mode: resolve selected preset's keywords and submit
      if (!selectedPresetId) return;
      const preset = resolvedPresets.find(p => p.id === selectedPresetId);
      if (!preset || preset.resolvedKeywords.length === 0) return;
      setIsSubmitting(true);
      await onAddBatchKeywords(preset.resolvedKeywords);
      setIsSubmitting(false);
      onClose();
    } else {
      // Favorites mode: existing behavior
      if (selectedTags.size === 0) return;
      setIsSubmitting(true);
      const selectedObjects = bankKeywords.filter(kw => selectedTags.has(kw.tag));
      await onAddBatchKeywords(selectedObjects);
      setIsSubmitting(false);
      onClose();
    }
  };
  ...
  const canSubmit = activeTab === 'presets' 
  ...
  const getFooterText = () => {
    if (isSubmitting) {
      if (activeTab === 'presets') {
        const preset = resolvedPresets.find(p => p.id === selectedPresetId);
        return `Adding ${preset?.tagCount || 0} keywords from preset...`;
      }
      return `Adding ${selectedTags.size} keywords...`;
    }
    if (activeTab === 'presets') {
      if (selectedPresetId) {
        const preset = resolvedPresets.find(p => p.id === selectedPresetId);
        return `Preset "${preset?.title}" — ${preset?.tagCount || 0} keywords`;
      }
      return 'Select a preset';
    }
    return `${selectedTags.size} selected`;
  };
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex border-b border-slate-100">
          <button
            ...
            className={`flex-1 py-2.5 text-sm font-bold text-center transition-all border-b-2 ${
              activeTab === 'favorites'
                ? 'text-indigo-700 border-indigo-600 bg-indigo-50/40'
                : 'text-slate-400 border-transparent hover:text-slate-600 hover:bg-slate-50'
            }`}
          >
            <Star size={13} className="inline mr-1.5 -mt-0.5" />
            Favorite Tags
          </button>
          <button
        ...
        {activeTab === 'presets' && (
          <div className="flex-1 overflow-y-auto min-h-0 px-2 py-2">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12">
                <Folder size={24} className="text-slate-200 mb-2" />
                <p className="text-sm text-slate-400">No presets yet. Create one from the SEO Lab or the Keyword Performance table.</p>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <Folder size={14} className={`shrink-0 ${isSelected ? 'text-indigo-600' : 'text-slate-400'}`} />
                          <span className={`text-sm font-bold truncate ${isSelected ? 'text-indigo-700' : 'text-slate-700'}`}>
                            {preset.title}
                          </span>
                        </div>
                        {(preset.theme || preset.niche) && (
                          <p className="text-[11px] text-slate-400 mt-0.5 ml-6 truncate">
                            {[preset.theme, preset.niche, preset.sub_niche].filter(Boolean).join(' › ')}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <span className={`flex items-center gap-1 text-xs font-mono ${isSelected ? 'text-indigo-500' : 'text-slate-400'}`}>
                          <Hash size={11} />
                          {preset.tagCount}
                        </span>
                        <span className={`flex items-center gap-1 text-xs font-mono ${isSelected ? 'text-indigo-500' : 'text-slate-400'}`}>
                          <BarChart3 size={11} />
                          {preset.totalVolume.toLocaleString()}
                        </span>
                      </div>
                    </div>
                  );
