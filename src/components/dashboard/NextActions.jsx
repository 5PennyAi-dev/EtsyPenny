...
function getContextualStat(key, count, { tokenBalance, avgScore }) {
  if (key === 'ANALYZED') {
    const cost = count * 8;
    const color = cost > tokenBalance ? 'text-amber-500' : 'text-slate-400';
    return <span className={`text-xs ${color} whitespace-nowrap`}>~{cost} tokens to generate all</span>;
  }
  if (key === 'SEO_READY' && avgScore != null) {
    return <span className="text-xs text-slate-400 whitespace-nowrap">avg score: {avgScore}</span>;
  }
  if (key === 'DRAFT_READY') {
    const cost = count * 1;
    const color = cost > tokenBalance ? 'text-amber-500' : 'text-slate-400';
    return <span className={`text-xs ${color} whitespace-nowrap`}>~{cost} token{cost > 1 ? 's' : ''} to generate all</span>;
  }
  return null;
}

export default function NextActions({ counts = {}, onNavigate, onNewListing, tokenBalance = 0, avgScore, lowestScore }) {
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-4">
      ...
      <div className="space-y-2">
        {actionableStatuses.map((key) => {
          ...
          const stat = getContextualStat(key, count, { tokenBalance, avgScore });

          return (
            <div
              key={key}
              className="flex items-center gap-3 py-2 px-2 rounded-lg hover:bg-slate-50 transition-colors relative"
            >
              <div
        ...
        {listingsBelow50 > 0 && (
          <div className="flex items-center gap-3 py-2 px-2 rounded-lg hover:bg-slate-50 transition-colors relative">
            ...
            {lowestScore != null && (
              <div className="absolute left-1/2 -translate-x-1/2 pointer-events-none">
                <span className="text-xs text-slate-400 whitespace-nowrap">lowest: {lowestScore}</span>
              </div>
            )}
            <div className="flex items-center gap-2 flex-shrink-0">
