...
- [Fix] **Competition score desync in reset-pool strength calculation** (2026-04-20, branch `feature/help-chatbot`): Fixed `runResetPool` so `listings_global_eval.listing_competition` (and the whole strength breakdown) is computed from **the exact same 13 keywords** that end up flagged `is_current_eval = true` in the UI. Previously, at [`lib/seo/run-reset-pool.ts`](lib/seo/run-reset-pool.ts), we captured `correctSelectedTags` from `applySEOFilter`'s output but then called `selectAndScore(filteredKeywords, finalParams)` on the **full ~40-keyword filtered pool**. `selectAndScore` re-ranks by composite score and picks a different top-13, so `strength.breakdown.competition` reflected a different 13 keywords than the ones shown/flagged. Symptom: after "Generate Keywords" the top-of-page Competition tile could read `10` (very low), but clicking "Recalculate Scores" bumped it correctly (recalculate-scores passes exactly the 13 displayed keywords with `ai_selection_count = 13`, so `selectAndScore` marks all 13 and computes strength on those 13). **Fix**: in `runResetPool`, filter down to `selectedKeywords = filteredKeywords.filter(k => k.is_selection_ia)` BEFORE calling `selectAndScore`, and override `ai_selection_count = selectedKeywords.length`. Mirrors recalculate-scores' behavior exactly and keeps the DB strength row aligned with the DB flag rows. Side benefit: Apply Strategy now also persists a consistent Competition score. 98/98 tests pass.
- [Fix] **Keyword pool race condition — synchronous reset-pool** (2026-04-20, branch `feature/help-chatbot`): Fixed a recurring bug where immediately after "Generate Keywords" the Studio briefly displayed the ENTIRE keyword pool (~50–100 rows) instead of the intended 25 (13 AI-selected + 12 next best). Only clicking Apply Strategy corrected the view. Root cause: the `save-seo` edge function was triggering `/api/seo/reset-pool` as fire-and-forget (`fetch(...).catch(...)`), and the frontend used a 1.5 s `setTimeout` to "hope" reset-pool finished before `handleLoadListing` queried `is_current_pool = true`. Network latency regularly pushed the pool-finalization past 1.5 s, so `handleLoadListing` saw all rows (which `persistSeo` had marked `is_current_pool: true` before narrowing). **Fix**: extracted the reset-pool body into a pure `runResetPool(listing_id, parameters)` function at [`lib/seo/run-reset-pool.ts`](lib/seo/run-reset-pool.ts). Both HTTP routes ([`api/seo/reset-pool.ts`](api/seo/reset-pool.ts) and `server.mjs`) are now thin wrappers. The `generate-keywords` route (api + server.mjs) and the Etsy scoring path ([`lib/etsy/score-etsy-listing.ts`](lib/etsy/score-etsy-listing.ts)) now `await runResetPool(...)` **inline** after `persistSeo`, before flipping `listings.status_id = SEO_DONE`. Deleted the fire-and-forget block from the `save-seo` edge function (the `RESET_POOL_API_URL` env var is no longer used) and removed the 1.5 s `setTimeout` band-aid in [`ProductStudio.jsx`](src/pages/ProductStudio.jsx). `persistSeo` still sends `trigger_reset_pool: true` to instruct the edge function to skip its status update — the generate-keywords route now owns the status transition so the realtime subscription in ProductStudio fires only after the pool is finalized. Net effect: one deterministic, awaited pipeline — no more race, no timing-based guessing. **Deploy note**: the `save-seo` edge function must be redeployed (`npx supabase functions deploy save-seo`) — deploy it first, then the Vercel functions + frontend. All 98 tests pass.
- [Safety] **Help chatbot system prompt — safety guardrails added** (2026-04-20, branch `feature/help-chatbot`): Added four rules to [`lib/help/system-prompt.ts`](lib/help/system-prompt.ts) — (a) no SEO outcome promises (ranking, sales, visibility increases), (b) escalate to `hello@pennyseo.ai` on bugs/payments/frustration/refund requests, (c) no actions on user's behalf — explain UI steps instead, (d) always confirm AI nature if asked; never claim to be human. Minimal surgical patch; no behavioral change expected for normal usage questions — guardrails apply to edge cases only. No other files touched (no migration, no route, no UI, no tests). Smoke test (5 manual questions) to be run by the user before beta rollout.
- [Feature] **Help Chatbot admin review UI** (2026-04-20, branch `feature/help-chatbot`, Prompt 4 of 4 — **initiative complete**): New "Help Conversations" accordion on `/admin/system` for reviewing user chatbot sessions and flagging documentation gaps. Placed directly below Beta Feedback, sky-accented with `MessagesSquare` icon. Features: 4-KPI strip (conversations, messages, avg msgs/conv, satisfaction rate), toolbar (search with 300ms debounce, feedback filter, date range 7d/30d/all), paginated table (25 rows/page), inline row expansion showing the full markdown-rendered conversation with feedback badges + optional user notes + copy-to-plaintext button. New components: `src/components/admin/HelpConversationsPanel.jsx` + `HelpConversationRow.jsx`. Lazy-loaded — only queries when the accordion opens (via controlled `isOpen`/`onToggle` on the `Accordion` component). Data path: **two `SECURITY DEFINER` Postgres RPC functions** instead of a view — `admin_list_help_conversations(date_from, filter, search, limit, offset)` (returns joined data + a `total_count` window for pagination) and `admin_get_help_conversation_stats(date_from)` (KPI row). The SECURITY DEFINER approach was required because joining `auth.users` for email needs elevated privileges that direct-from-browser `authenticated` queries don't have. Admin check inside each function body uses the role-based `EXISTS ON profiles.role = 'admin'` pattern (matching Prompt 2's RLS policies; NOT the JWT-email pattern the original spec assumed). Migration: `supabase/migrations/20260420_add_admin_help_conversations_fn.sql` (applied). Non-admin callers get a PG `42501 insufficient_privilege` error back. Individual message fetching (for row expansion) uses direct `supabase.from('help_messages')` queries gated by the admin-read RLS policy from Prompt 2. Tests: `tests/integration/admin-help-conversations.test.ts` (5 cases, RPC contract coverage). Full suite 98/98 pass; `npm run build` passes.

  **Closed-loop workflow for the team:** open the Help Conversations accordion weekly → set filter to "With negative feedback" → review each flagged conversation → identify the recurring documentation gap → update the relevant `src/content/docs/**/*.md` article → `npm run build` (regenerates corpus) → redeploy. The bot automatically grounds on the refreshed corpus on the next conversation, no code change needed.

  **Help Chatbot initiative is now complete across all four prompts**: docs migration (Prompt 1), backend + SSE streaming (Prompt 2), user-facing UI (Prompt 3), admin review (Prompt 4). No follow-up pieces required before shipping this feature; chart/CSV/auto-gap-detection features are explicitly deferred until real usage data informs their design.
- [Feature] **Help Chatbot frontend UI** (2026-04-20, branch `feature/help-chatbot`, Prompt 3 of 4): Added the user-facing chat UI on top of the Prompt 2 backend. New `src/context/HelpChatContext.jsx` holds the chatbot state (open/closed, conversationId, messages, isSending, rateLimitInfo, conversations list) plus actions: `open/close/toggle`, `sendMessage`, `retryLastMessage`, `newConversation`, `loadConversation`, `loadConversationList`, `submitFeedback`, `abort`. The context also exports a pure `consumeSseStream(reader, onChunk, { signal })` function that reads the `/api/help/chat` ReadableStream, reassembles SSE frames across packet boundaries, and parses each `data: ...` JSON chunk. New components under `src/components/help/`: `HelpChatBubble.jsx` (fixed bottom-right indigo button with one-time `animate-ping` halo per session, suppressed under `prefers-reduced-motion`), `HelpChatPanel.jsx` (desktop 400×600 card bottom-right / mobile fullscreen with ESC + outside-click close, auto-scroll on new deltas, auto-grow textarea, rate-limit banner at ≥80% usage, three example-prompt chips in the welcome state), `HelpMessage.jsx` (user vs. assistant bubbles; assistant content rendered via react-markdown with a compact component set; streaming 3-dot pulse before first delta; retry button on error; thumbs up/down feedback on completed assistant messages with an optional note textarea after a thumbs-down), `HelpConversationList.jsx` (last 20 conversations via Supabase RLS + relative-time labels). Integrated into `src/components/Layout.jsx`: wraps children in `<HelpChatProvider>` and mounts `<HelpChatBubble />` + `<HelpChatPanel />` at the bottom of the layout. Automatically excluded from Landing, Login, Pricing, Privacy, Terms (they don't use `<Layout>`). Auth: `user_id` from `AuthContext.user.id` in the request body (matches Prompt 2 backend — no JWT Bearer). Counter UX: per-session user-message count shown under the input; when the backend returns 429, the real `{ used, limit, resetAt }` takes over and the input is disabled. History trim: last 10 message turns sent server-side. Page context: `window.location.pathname` at send time. Tests: `tests/unit/help-chat-sse.test.ts` (7 cases) verifies the SSE parser against packet splits, multi-frame chunks, malformed frames, comment lines, aborts, and trailing-frame flush. Full suite 93/93 pass; build passes. Frontend tests were scoped to the parser only (RTL + happy-dom are not installed; the state logic has good unit coverage via the parser which is the only non-trivial code path). **Admin review UI is still pending (Prompt 4).**
- [Feature] **Help Chatbot backend** (2026-04-20, branch `feature/help-chatbot`, Prompt 2 of 4): Added the server side of the in-app AI help assistant. New Supabase tables `help_conversations` and `help_messages` with per-user RLS and admin read-only override (role-based EXISTS pattern, matching the existing project convention — NOT JWT-email). New routes `POST /api/help/chat` (Server-Sent Events streaming, grounded on the Markdown corpus from Prompt 1) and `POST /api/help/feedback` (thumbs up/down per assistant message), mirrored in `server.mjs` for local dev. Rate limit: 20 user messages per rolling 24h (`HELP_DAILY_LIMIT`, queried from `help_messages`). Corpus is delivered at build time: `scripts/build-help-corpus.mjs` concatenates `src/content/docs/**/*.md` into `lib/help/corpus-data.ts` (committed, 54k chars across 12 articles); wired into `prebuild` and `predev:api` npm hooks so Vercel serverless functions can `import` it directly without runtime `fs.readFile`. Extended the AI adapter: new `streamAI(taskKey, messages, options)` in `lib/ai/provider-router.ts` + `callGeminiStream()` in `lib/ai/adapters/gemini-adapter.ts` (no `responseMimeType: 'application/json'` on the streaming path — plain text). The existing non-streaming `runAI()` is unchanged. New `lib/help/` modules: `corpus.ts`, `system-prompt.ts` (bilingual FR/EN prompt), `rate-limit.ts`, `chat-service.ts` (async generator orchestrating user-message persistence → stream → assistant-message persistence). Auth: `user_id` in request body (consistent with every other `api/*` route — no JWT Bearer). Task config: one row added to `system_ai_config` with `task_key='help_chat'`, `gemini-2.5-flash`, `temperature=0.6`, `max_tokens=1024`. Tests: `tests/integration/help-chat.test.ts` (6 cases) + `tests/help-chat.http` (8-scenario curl cookbook). Extended `tests/mocks/supabase.ts` so `.insert().select().single()` works chainably. Full suite: 86/86 pass. **Frontend chatbot UI is Prompt 3 — not yet implemented.**
- [Refactor] **Docs → Markdown corpus** (2026-04-19, branch `feature/help-chatbot`): Migrated all `/docs/*` articles from hand-written JSX to a single Markdown corpus under `src/content/docs/` (12 files, YAML frontmatter: slug/title/category/order/icon/summary/updated). New `src/components/docs/MarkdownArticle.jsx` loads the corpus via Vite `import.meta.glob('?raw')`, parses with `gray-matter`, and renders via `react-markdown` + `remark-gfm` + `remark-directive`. Container directives (`:::tip`, `:::warning`, `:::howto`, `:::section{label title}`, `:::deflist{icon}`, `:::faq`) preserve the original visual components; inline `:IconName:` markers handle per-item icons (Flame/Leaf/Star in smart-badge lists). Collapsed 13 `<Route>` entries in `App.jsx` to a single catch-all under `/docs/*`. `DocsLayout.jsx` now derives sidebar navigation from `getAllArticles()` grouped by `frontmatter.category` — adding a new `.md` file automatically lands in the nav. Deleted `src/pages/docs/*.jsx` (13 files), `src/components/docs/DocComponents.jsx`, `src/config/docsNavigation.js`. Renamed two silently-broken screenshots in `public/docs/` (`keywords-table.png .png` → `keywords-table.png`, `keywords-strategy-tuner.png .png` → same fix). Added `scripts/validate-docs-corpus.mjs` and `scripts/validate-docs-parsing.mjs` to keep the corpus healthy. **This is Step 1 of 4** for the Help Chatbot initiative — the Markdown corpus now serves as a single source of truth consumable by both the UI and a future LLM. No chatbot logic touched in this step.
